...
for dest in ecos:
    ...
    for src in ecos:
        print "   Scanning",src
        inflf = csv.DictReader(gzip.open("data/infl_pkg_" + src + "_to_" + dest + "_0.csv.gz"))
        for row in inflf:
            pkgs[row["package"]][row["year"]] += int(row["influence"])
    totalsf = csv.writer(gzip.open("data/infl_total_pkg_" + dest + ".csv.gz","wb"))
    ...
    for src in ecos:
        print "   Rewriting",src
        inflf = csv.DictReader(gzip.open("data/infl_pkg_" + src + "_to_" + dest + "_0.csv.gz"))
        outflf = csv.writer(gzip.open("data/infl_pkg_normalized_" + src + "_to_" + dest + ".csv.gz", "wb"))
